...
public class MyArraysIn {
	...
	public static int[] removeNumber(int[] array, int index) {
		//todo
		//removes element at a given index for right index value (0 - array.length-1) and returns new array without the remove one
		//returns the source array for a wrong index value (less then 0 or greater then array.length-1
	...
	public static int[] insertNumber(int[] array, int index, int num) {
		//TODO
		//inserts a given num at a given index for right value of the index (0 - array.length) and returns new array with the inserting number
	...
	public static int binarySearch(int[] array, int key) {
		...
		Arrays.sort(array);
		if (Arrays.binarySearch(array, key)>=0) return 1;
		else
		return -1;
	}
}
